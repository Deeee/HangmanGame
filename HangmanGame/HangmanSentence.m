...
@implementation HangmanSentence
...
@synthesize mode;
@synthesize sentenceStatus;
@synthesize knowSentence;
...
-(void) analysisServer:(NSString *)state withCode:(int) code{
    sentenceStatus = state;
    knowSentence = [[NSMutableString alloc] initWithString:sentenceStatus];
    [knowSentence replaceOccurrencesOfString:@"_" withString:@"?" options:NSCaseInsensitiveSearch range:NSMakeRange(0, [knowSentence length])];
    ...
    if (code == 1) {
        for (NSString *str in words) {
            ...
            NSLog(@"%@ added to guessArray",s);
        }

        NSLog(@"here state is %@, knowsentence is %@",state, knowSentence);
        return;
    }
    int count = 0;
    for (NSString *str in words) {
    ...
    sentenceStatus = state;
    knowSentence = [[NSMutableString alloc] initWithString:sentenceStatus];
    [knowSentence replaceOccurrencesOfString:@"_" withString:@"?" options:NSCaseInsensitiveSearch range:NSMakeRange(0, [knowSentence length] - 1)];
    NSLog(@"here state is %@, knowsentence is %@",state, knowSentence);
...
-(NSString *) serverInitialize:(NSString *) input{
    ...
    [self analysisServer:input withCode:1];
    ...
    for (NSString *word in guessArray) {
        ...
        NSLog(@"filtering word %@, length %ld, filter array count %ld",word, length, [filterArray count]);
